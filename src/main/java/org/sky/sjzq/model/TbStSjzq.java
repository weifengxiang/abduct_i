package org.sky.sjzq.model;

import java.util.Date;

public class TbStSjzq {
    private String id;

    private String xqlb;

    private String xqbh;

    private String xm;

    private String xb;

    private String csrq;

    private String szssg;

    private String szsj;

    private String szrszd;

    private String szdd;

    private String xqztzms;

    private String qtzl;

    private String zcsj;

    private String gjzyz;

    private String yswz;

    private Date createTime;

    private String note;
    
    private String tx;

    public String getId() {
        return id;
    }

    public void setId(String id) {
        this.id = id;
    }

    public String getXqlb() {
        return xqlb;
    }

    public void setXqlb(String xqlb) {
        this.xqlb = xqlb;
    }

    public String getXqbh() {
        return xqbh;
    }

    public void setXqbh(String xqbh) {
        this.xqbh = xqbh;
    }

    public String getXm() {
        return xm;
    }

    public void setXm(String xm) {
        this.xm = xm;
    }

    public String getXb() {
        return xb;
    }

    public void setXb(String xb) {
        this.xb = xb;
    }

    public String getCsrq() {
        return csrq;
    }

    public void setCsrq(String csrq) {
        this.csrq = csrq;
    }

    public String getSzssg() {
        return szssg;
    }

    public void setSzssg(String szssg) {
        this.szssg = szssg;
    }

    public String getSzsj() {
        return szsj;
    }

    public void setSzsj(String szsj) {
        this.szsj = szsj;
    }

    public String getSzrszd() {
        return szrszd;
    }

    public void setSzrszd(String szrszd) {
        this.szrszd = szrszd;
    }

    public String getSzdd() {
        return szdd;
    }

    public void setSzdd(String szdd) {
        this.szdd = szdd;
    }

    public String getXqztzms() {
        return xqztzms;
    }

    public void setXqztzms(String xqztzms) {
        this.xqztzms = xqztzms;
    }

    public String getQtzl() {
        return qtzl;
    }

    public void setQtzl(String qtzl) {
        this.qtzl = qtzl;
    }

    public String getZcsj() {
        return zcsj;
    }

    public void setZcsj(String zcsj) {
        this.zcsj = zcsj;
    }

    public String getGjzyz() {
        return gjzyz;
    }

    public void setGjzyz(String gjzyz) {
        this.gjzyz = gjzyz;
    }

    public String getYswz() {
        return yswz;
    }

    public void setYswz(String yswz) {
        this.yswz = yswz;
    }

    public Date getCreateTime() {
        return createTime;
    }

    public void setCreateTime(Date createTime) {
        this.createTime = createTime;
    }

    public String getNote() {
        return note;
    }

    public void setNote(String note) {
        this.note = note;
    }

	public String getTx() {
		return tx;
	}

	public void setTx(String tx) {
		this.tx = tx;
	}
}